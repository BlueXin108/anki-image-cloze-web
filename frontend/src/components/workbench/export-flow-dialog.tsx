...
import {ExportSuccessFullscreen} from "@/components/workbench/export-success-fullscreen";
import {
	Card,
	CardContent,
	CardDescription,
...
type ExportFlowStage = "review" | "confirm" | "success";
...
function truncatedImageLabel(item: DraftListItem, maxLength = 42): string {
	...
	const headLength = Math.max(
		10,
		Math.floor((maxLength - extension.length - 1) * 0.6),
	);
	...
	const steps = [
		{
			...
			label: "选择牌组",
			active: stage === "review",
			done: reviewedCount > 0 || stage === "confirm" || stage === "success",
		},
		{
			id: 2,
			label: "确认图片",
			active: stage === "review",
			done: stage === "confirm" || stage === "success",
		},
		{id: 3, label: "最终导出", active: stage === "confirm", done: stage === "success"},
	];
...
function FlowProgressCard({
	...
	const progress =
		stage === "review"
			? Math.max(10, Math.round((reviewedCount / Math.max(total, 1)) * 70))
			: 100;
	...
	return (
		<div
			className={cn(
				...
				compact
					? "max-w-[12rem] gap-1.5 px-2.5 py-2 sm:min-w-[11rem]"
					: "max-w-[16rem] gap-2 px-3 py-2 sm:min-w-[14rem]",
			)}>
			<div
				className={cn(
			...
			<Progress
				value={progress}
				className={cn("bg-muted/40", compact ? "h-1" : "h-1.5")}
			/>
		</div>
	);
}
		...
		<div className="relative flex-1">
			<button
				...
				className={cn(
					...
					disabled &&
						"cursor-not-allowed border-border/50 bg-muted/30 text-muted-foreground hover:border-border/50",
				)}>
				{label}
			</button>
			{disabled && reason ? (
...
function ThumbnailQueue({
	...
	useEffect(() => {
		...
		const target = containerRef.current?.querySelector<HTMLButtonElement>(
			`[data-draft-id="${current.draft.id}"]`,
		);
			...
			<div className="relative overflow-hidden rounded-xl border border-border/60 bg-muted/10">
				...
				<div
					...
					{queue.map((item, index) => {
						...
						const isDone =
							reviewed.has(item.draft.id) ||
							(isCurrent ? !!currentDeckInput?.trim() : hasDraftDeck);
						return (
							<motion.button
								layout
								type="button"
								...
								<div
									className={cn(
										"relative z-10 mt-1 max-w-full truncate text-muted-foreground",
...
function ExportPreviewPane({
	...
	if (compact) {
		return (
			<>
				<Card className="border-none outline-none ring-0 bg-background/92 shadow-none overflow-hidden">
					<CardHeader className="gap-2 px-3 py-2.5 bg-muted/5">
						<div className="flex items-center justify-between gap-3">
							<div className="min-w-0 flex-1">
								<CardTitle
									className="truncate text-[13px] font-medium leading-relaxed tracking-wide text-foreground/90"
									title={item.image.source_path}>
							...
							<Button
								size="sm"
								// 核心：使用 inline-flex 确保按钮宽度由内容决定，或用 w-full 配合 justify-center
								className="h-9 rounded-xl px-3 text-[12px] font-medium bg-primary/95 text-primary-foreground shadow-sm hover:bg-primary flex items-center justify-center"
								onClick={() => setFocusOpen(true)}>
								{/* 使用一个容器包裹内容，确保内部绝对居中 */}
								<div className="text-center flex items-center justify-center gap-1.5">
									<SquarePenIcon className="size-3.5" />
									<div text-center>聚焦编辑</div>
								</div>
							</Button>
						</div>
					</CardHeader>
	...
	return (
		<>
			<Card className="flex h-full flex-col border-none outline-none ring-0 bg-background/90 shadow-none max-h-[75vh]">
				<CardHeader className="gap-2 pb-0">
					<div className="flex items-start justify-between gap-3">
						<div className="space-y-1">
							<CardTitle className="truncate" title={item.image.source_path}>
								{truncatedImageLabel(item, 52)}
							</CardTitle>
							<CardDescription>
								此处确认编辑， <Kbd>Q</Kbd> 可直接打开聚焦编辑。
							</CardDescription>
						</div>
						<Button
							variant="default"
							size="sm"
							className={cn(
								"rounded-full shadow-sm flex items-center justify-center gap-1.5 text-center shrink-0",
								touchOptimized ? "h-10 px-5" : "h-9 px-3", // 微调 px 确保两侧间距对称
							)}
							onClick={() => setFocusOpen(true)}>
							{/* 1. 确保图标类名干净，不受外部 data-icon 样式干扰 */}
							...
							{/* 2. 文字包裹在 span 中，防止匿名文本节点产生的间距偏移 */}
							<span className="leading-none">聚焦编辑（Q）</span>
						</Button>
					</div>
				</CardHeader>
...
export function ExportFlowDialog({
	...
	const [exportTarget, setExportTarget] = useState<
		"anki" | "apkg" | "image-group"
	>(allowDirectAnki ? "anki" : "apkg");
	...
	const totalCards = queue.reduce(
		(sum, item) => sum + countGeneratedCards(item.draft, generationMode),
		0,
	);
	const readyCount = queue.filter((item) => item.draft.deck?.trim()).length;
	const dialogShellSize = isMobileDialog
		? {
		...
		: stage === "review"
			...
			: stage === "confirm"
				? {
						width: Math.max(
							...
							Math.min(
								Math.round(
									viewportSize.width * (viewportSize.width >= 640 ? 0.88 : 0.92),
								),
								832,
							),
						),
			...
			: {
					width: Math.max(
						...
						Math.min(
							Math.round(
								viewportSize.width * (viewportSize.width >= 960 ? 0.48 : 0.62),
							),
							680,
						),
					),
					height: Math.max(440, Math.min(Math.round(viewportSize.height * 0.72), 620)),
				};
	const showReviewPreview = stage === "review";
	const isExporting =
		isExportingAnki || isExportingApkg || isExportingImageGroup;
	const exportedCount = exportedDraftIds.length;
	const activeExportTarget = allowDirectAnki
		? exportTarget === "anki" && !canDirectAnki
			? "apkg"
			: exportTarget
		: exportTarget === "anki"
			? "apkg"
	...
	const activeExportBusy =
		activeExportTarget === "anki"
			...
			: activeExportTarget === "image-group"
				? isExportingImageGroup
				: isExportingApkg;
	const effectiveFormat: ImageExportFormat = allowedExportFormats.includes(
		imageGroupFormat,
	)
		? imageGroupFormat
		: (allowedExportFormats[0] ?? "webp");
	const previewOutputType =
	...
	const qualityValue =
		activeExportTarget === "image-group" ? imageGroupQuality : quality;
							...
							<div
								...
								<motion.div
									...
									<div
										...
										<DialogHeader
											className={cn(
												"shrink-0 border-b border-border/60 bg-popover",
												isMobileDialog ? "px-4 py-3" : "px-6 py-4 mt-2",
											)}>
											<div className="flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between">
												<div className="min-w-0">
													<DialogTitle
														className={cn(
															isMobileDialog && "text-[15px] font-medium",
														)}>
														导出当前项目
													</DialogTitle>
													<DialogDescription
										...
										<div
											className={cn(
												"min-h-0 flex-1 overflow-y-auto overflow-x-hidden bg-muted/5 grid",
												isMobileDialog
													? "p-3 [&::-webkit-scrollbar]:hidden [scrollbar-width:none]"
													: "p-4 sm:p-6",
											)}>
											<AnimatePresence initial={false} mode="wait">
												{stage === "review" ? (
													...
													<motion.div
														...
														initial={{
															opacity: 0,
															x: isMobileDialog ? 0 : -15,
															y: isMobileDialog ? 10 : 0,
														}}
														...
														transition={{
															duration: isMobileDialog ? 0.15 : 0.2,
															ease: [0, 0.43, 0, 0.99],
														}}
														style={{gridArea: "1 / 1"}}
														className={cn(
											...
											onWorkbenchSettingsChange={onWorkbenchSettingsChange}
															...
															<Card
																...
																<CardHeader
																	className={cn(
																		"gap-4 pb-3",
																		isMobileDialog && "gap-3 px-3 py-3",
																	)}>
																...
																<CardContent
																	...
																	<div
																		className={cn(
																			"flex flex-col gap-4 rounded-2xl border border-border/60 bg-muted/10",
																			isMobileDialog ? "p-4.5" : "p-4",
																		)}>
																		<DeckPicker
																			decks={deckOptions}
																		...
																		/>
																		<div className="h-px bg-border/60 -mx-1 hidden" />
																				...
																				<FieldContent>
																					<Input
																						...
																						onFocus={(e) => {
																							...
																							const scrollBox = target.closest(
																								".overflow-y-auto",
																							) as HTMLElement;
																							if (scrollBox) {
																								setTimeout(() => {
																	...
																	{isMobileDialog && keyboardPad && (
																		<div className="h-40 shrink-0 pointer-events-none trs-all-400" />
																	)}
													...
													<motion.div
														...
														<Card
															className={cn(
																"flex w-full max-w-2xl mx-auto flex-col border-border/70 bg-background/94 shadow-none",
																!isMobileDialog && "h-full",
															)}>
															<CardHeader
																className={cn(
																	"gap-3 pb-3",
																	isMobileDialog && "gap-2 px-4 pt-3 pb-0",
																)}>
																<div className="mx-auto w-full max-w-md">
																	<StepRail
																		stage={stage}
															...
															<CardContent
																...
																<div
																	className={cn(
																		"flex flex-col",
																		isMobileDialog ? "gap-3" : "h-full gap-3",
																	)}>
																	<div className="flex flex-col gap-2 rounded-xl border border-border/50 bg-muted/10 p-3">
																		...
																		<ToggleGroup
																			...
																			spacing={1}
																			className={cn(
																				"w-full rounded-lg bg-background/70 p-1",
																				allowDirectAnki
																					? "grid grid-cols-3"
																					: "grid grid-cols-2",
																			)}>
																		...
																		<div
																			className={cn(
																				"text-[11.5px] mt-1 text-muted-foreground leading-relaxed pl-1",
																				isMobileDialog && "text-[10px]",
																			)}>
																			{allowDirectAnki ? (
																				<>
																					桌面上可{" "}
																					<InlineEmphasis
																						onClick={onOpenAnkiHelp}>
																						直连 Anki 写入
																					</InlineEmphasis>
																					，也可以下载卡包或图像组。
																				</>
																		...
																		<div className="mt-auto flex flex-col sm:flex-row sm:items-center justify-between gap-3 pt-2">
																			...
																			<div className="flex shrink-0 items-center justify-end gap-2">
																				<Button
																					variant="outline"
																					size="sm"
																					className="trs-all-400 h-9 font-medium hover:-translate-y-0.5 active:scale-[0.98]"
																				...
																				<Button
																					size="sm"
																					className="trs-all-400 h-9 px-4 border border-transparent font-medium hover:-translate-y-0.5 active:scale-[0.98] hover:bg-background hover:text-primary hover:border-border"
																					onClick={activeExportAction}
																					disabled={isExporting}>
																					{activeExportBusy ? (
													...
													{stage === "review" ? (
														<motion.div
															...
															<div className="flex items-center justify-between text-[11px] text-muted-foreground">
																...
																<Badge
																	variant="outline"
																	className="rounded-full px-2 py-0 text-[10px] bg-background">
																	{currentIndex + 1}/{queue.length}
																</Badge>
															</div>
															<div className="grid grid-cols-3 gap-2">
																<Button
																	variant="outline"
																	className="trs-all-400 h-10 rounded-xl px-2 text-[12px] font-medium tracking-wide hover:-translate-y-0.5 active:scale-95 active:bg-foreground active:text-background"
																	onClick={onPrevious}
																	disabled={currentIndex === 0}>
																	上一张
																</Button>
																...
																<Button
																	className="trs-all-400 h-10 rounded-xl px-2 text-[12px] font-medium tracking-wide bg-foreground text-background hover:-translate-y-0.5 hover:bg-foreground/90 active:scale-95 active:bg-background active:text-foreground active:border active:border-foreground"
																	onClick={onConfirmCurrent}
																	disabled={!deckInput.trim()}>
																	<CheckCheckIcon
														...
														<motion.div
															...
															<div className="grid grid-cols-2 gap-2">
																<Button
																	variant="outline"
																	className="trs-all-400 h-10 rounded-xl px-3 text-[12px] font-medium tracking-wide hover:-translate-y-0.5 active:scale-95 active:bg-foreground active:text-background"
																	onClick={onBackToReview}
																	disabled={isExporting}>
																	<ChevronLeftIcon
																		data-icon="inline-start"
																...
																<Button
																	className="trs-all-400 h-10 rounded-xl px-3 border border-transparent text-[12px] font-medium tracking-wide hover:-translate-y-0.5 hover:bg-background hover:text-primary hover:border-border active:scale-95 active:bg-background active:text-foreground active:border-foreground"
																	onClick={activeExportAction}
																	disabled={isExporting}>
																	{activeExportBusy ? (
																		<Spinner data-icon="inline-start" />
